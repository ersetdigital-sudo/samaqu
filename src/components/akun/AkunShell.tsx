  ...
  return (
    <div className="akun-root min-h-screen" style={{ background: "#f5f1ea" }}>
      ...
      <div className="hidden md:block">
        ...
        <aside className={`fixed inset-y-0 left-0 w-64 bg-white border-r border-[#e6dfd2] z-50 flex flex-col transition-transform duration-200 ${sidebarOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"}`}>
          {/* Sidebar header */}
        ...
        <div className="md:ml-64 flex flex-col min-h-screen">
          {/* ── Sticky header ── */}
          <header className="bg-white border-b border-[#e6dfd2] sticky top-0 z-30">
            <div className="h-16 flex items-center gap-3 px-4 sm:px-6">
              <button className="md:hidden text-[#6c7a75] hover:text-[#0f3d33] text-xl" onClick={() => setSidebarOpen(true)}>
                <svg viewBox="0 0 24 24" className="w-6 h-6" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M4 6h16M4 12h16M4 18h16" /></svg>
              </button>
          ...
          {/* ── Footer ── */}
          <footer className="bg-white border-t border-[#e6dfd2] py-6">
...
export function AkunHeader({ title, back }: { title: string; back?: string }) {
  ...
  return (
    <div className="akun-root md:hidden sticky top-0 z-30 -mx-4 px-4 bg-[#f5f1ea]/90 backdrop-blur flex items-center gap-3 pt-3 pb-4 border-b border-[#eae3d6]">
      <button
        type="button"
