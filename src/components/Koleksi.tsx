...
export default function Koleksi() {
  return (
    <section id="produk" className="py-24 sm:py-32" style={{ background: "var(--sand-2)" }}>
      <div className="max-w-[1200px] mx-auto px-5 sm:px-8">
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mb-14 fade-up">
          ...
          <p className="leading-[1.75] max-w-sm text-sm" style={{ color: "var(--coffee)" }}>
            Enam kategori inti yang dirancang untuk melengkapi setiap momen — ibadah, keseharian, hingga acara istimewa.
          </p>
        </div>
          ...
          {products.map((product) =>
            product.isFeature ? (
              <a
                ...
                target="_blank"
                rel="noopener"
                className="group relative col-span-2 lg:col-span-2 lg:row-span-2 overflow-hidden rounded-[2px]"
              >
                <Image
                  src={product.src}
                  alt={product.alt}
                  width={800}
                  height={600}
                  className="card-img w-full h-64 lg:h-full object-cover"
                />
                <div className="absolute inset-0" style={{ background: "linear-gradient(180deg,rgba(43,38,32,0) 40%,rgba(43,38,32,.75))" }} />
                <div className="absolute bottom-0 left-0 p-7">
                  <p className="text-[11px] tracking-[0.32em] uppercase mb-1" style={{ color: "var(--sand)" }}>
                    {product.badge}
                  </p>
                  <h3 className="text-3xl text-white mb-1" style={{ fontFamily: "var(--font-cormorant), Georgia, serif" }}>
                    {product.name}
                  </h3>
                  <p className="text-sm" style={{ color: "rgba(216,196,168,.9)" }}>
                    {product.desc}
                  </p>
                </div>
              </a>
              ...
              >
                <Image
                  ...
                  width={400}
                  height={300}
                  className="card-img w-full h-44 sm:h-56 object-cover"
                />
                <div className="absolute inset-0" style={{ background: "linear-gradient(180deg,rgba(43,38,32,0) 45%,rgba(43,38,32,.7))" }} />
                <div className="absolute bottom-0 left-0 p-5">
                  <h3 className="text-2xl text-white" style={{ fontFamily: "var(--font-cormorant), Georgia, serif" }}>
                    {product.name}
                  </h3>
                </div>
              </a>
