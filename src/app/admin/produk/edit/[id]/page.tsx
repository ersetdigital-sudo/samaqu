                ...
                <div className="space-y-3">
                  ...
                  {activeVariant.sizes.map((s, i) => (
                    <div key={i} className="flex items-center gap-3 flex-wrap">
                      ...
                      <input value={s.priceOverride} onChange={(e) => updateSizeField(activeColor!, i, "priceOverride", e.target.value)} placeholder="Kosongkan jika sama" className="w-36 rounded-lg px-3 py-2 text-sm outline-none" style={{ border: "1px solid rgba(64,50,37,.15)", background: "white", color: "var(--espresso)" }} />
          ...
          <div className="hidden lg:block sticky top-24">
            <div className="card p-5">
              ...
              <div className="aspect-[4/5] rounded-xl overflow-hidden mb-4" style={{ background: "#e8dfd1" }}>
                {media.find((m) => m.url && !m.isVideo) ? <img src={media.find((m) => m.url && !m.isVideo)?.url} alt="" className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center"><ImageIcon size={32} style={{ color: "var(--text-muted)" }} /></div>}
              </div>
              ...
              <p className="text-lg font-serif italic mt-2" style={{ color: "var(--gold)" }}>{basePrice ? `Rp ${parseInt(basePrice).toLocaleString("id-ID")}` : "Rp 0"}</p>
            </div>
          </div>
        </div>
      </main>
